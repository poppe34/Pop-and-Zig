...
// Add Radius
	SET_FRAME_DATA(fr->nwk, 0x06, 1);//TODO: this value should be temp I need a variable in its place

// Add Source Addr
	SET_FRAME_DATA(fr->nwk, npdu->source.shortAddr, 2);

// Add Dest Addr
	SET_FRAME_DATA(fr->nwk, npdu->destination.shortAddr, 2);

// Add NWK fcf
	uint16_t *tempfcf = &npdu->fcf;
	SET_FRAME_DATA(fr->nwk, *tempfcf, 2);
