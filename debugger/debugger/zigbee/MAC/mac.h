...
typedef struct PACK MAC_FCF{
	COMBINE(6, (
	uint16_t 	MAC_fcf_Frame_Type: 3,
	uint16_t 	MAC_fcf_Sec_enabled: 1,
	uint16_t 	MAC_fcf_Frame_Pending: 1,
	uint16_t 	MAC_fcf_Ack_Request: 1,
	uint16_t	MAC_fcf_PANid_Compression: 1,
	uint16_t	res1: 1))
	COMBINE(4, (
	uint16_t	res2: 2,
	uint16_t	MAC_fcf_DstAddr_Mode: 2,
	uint16_t	MAC_fcf_Frame_Ver: 2,
	uint16_t	MAC_fcf_SrcAddr_Mode: 2))
}mac_fcf_t;
END_PACK
